...
/*----- Estructura que maneja el exit status del thread ---------*/
struct exit_status
{
	tid_t cid; //id del child process
	struct lock *exitLock;//Lock de la lista
  struct semaphore *semaphore_wait;
  struct list_elem child_elem; //elemento de la lista de exit status
};
